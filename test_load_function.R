# Test script untuk fungsi load_shapefile_data
library(sf)

# Definisi fungsi load_shapefile_data
...
# Test fungsi
cat("Testing load_shapefile_data function...\n")
result <- load_shapefile_data()

if (!is.null(result)) {
  cat("✅ Function works! Data loaded successfully\n")
  cat("Rows:", nrow(result), "\n")
  cat("Columns:", ncol(result), "\n")
  cat("Sample columns:", paste(head(names(result), 10), collapse = ", "), "\n")
  
  if ("DISTRICTCODE" %in% names(result) && "Longitude" %in% names(result)) {
    cat("✅ Required columns (DISTRICTCODE, Longitude, Latitude) present\n")
  } else {
    cat("❌ Missing required columns\n")
  }
} else {
  cat("❌ Function failed to load data\n")
}